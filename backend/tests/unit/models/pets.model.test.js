...
describe('Model -> Testes de unidade do model de pets', function () {
  ...
  it.only('Listando todos os pets', async function () {
    ...
    it('Listando todos os pets e filtrando com a palavra escolhida', async function () {
    // Arrange
    sinon.stub(connection, 'execute').resolves([petsMockModel]);
    // Act
    const result = await petModel.searchProduct('M');
    // Assert
    expect(result).to.be.deep.equal([petsMockModel[0]]);
  });
  ...
  it('Recuperando um produto a partir do seu id', async function () {
    ...
    // Act
    const result = await petModel.getId(2);
    // Assert
